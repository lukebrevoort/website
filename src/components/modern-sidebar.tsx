...
export function ModernSidebar({
  ...
  return (
    <div
      className={`sticky top-0 h-screen backdrop-blur-xl bg-white/10 border-r border-white/20 transition-all duration-300 ease-in-out flex flex-col shadow-2xl shadow-black/5 z-40 ${
    ...
    >
      ...
      <div className="p-4 border-b border-white/10 relative">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            {!isCollapsed && (
              <div
                className={`flex flex-col opacity-0 animate-fadeIn ${lukesFont.className}`}
                style={{
                  animationDelay: "0.2s",
              ...
              >
                <span
                  className={`text-black-300 text-lg ${lukesFont.className}`}
                >
                  Luke Brevoort
                </span>
                <span
          ...
          <div key={section.title} className="mb-6">
            {!isCollapsed && (
              <div className="px-4 mb-2">
                <h3
                  className="text-sm uppercase tracking-wider text-indigo-400 font-medium opacity-0 animate-fadeIn"
                  style={{
                    animationDelay: `${0.3 + sectionIndex * 0.1}s`,
                    animationFillMode: "forwards",
                  }}
                >
                  {section.title}
                ...
                return (
                  <div key={item.href} className="relative group">
                    ...
                    <Button
                      ...
                      style={{
                        animationDelay: `${0.4 + sectionIndex * 0.1 + itemIndex * 0.05}s`,
                      }}
                    >
                      <div
                        className={`${isCollapsed ? "opacity-0 animate-fadeIn" : ""}`}
                        style={
                          isCollapsed
                            ? {
                      ...
                      >
                        <Icon
                          className={`h-4 w-4 ${isCollapsed ? "" : "mr-3"} transition-all duration-300 ${
                            isActive
                      ...
                      {!isCollapsed && (
                        <div
                          className="flex-1 flex items-center justify-between opacity-0 animate-fadeIn"
                          style={{
                            animationDelay: `${0.5 + itemIndex * 0.05}s`,
                            animationFillMode: "forwards",
                          }}
                        >
                          ...
                          <div className="flex items-center space-x-1">
                            {item.badge && (
                              <Badge
                                variant="secondary"
                                className="text-xs px-1.5 py-0.5 h-5 bg-gradient-to-r from-emerald-500/20 to-blue-500/20 border-white/20 text-white/80 backdrop-blur-sm"
                              >
                                {item.badge}
                              </Badge>
                            )}
                            <ChevronRight
                              className={`h-3 w-3 transition-all duration-300 group-hover:translate-x-0.5 ${
                                isActive
            ...
            {sectionIndex < navSections.length - 1 && !isCollapsed && (
              <Separator
                className="mx-4 mt-4 bg-white/10 opacity-0 animate-fadeIn"
                style={{
                  animationDelay: `${0.6 + sectionIndex * 0.1}s`,
                  animationFillMode: "forwards",
                }}
              />
            )}
      ...
      {!isCollapsed && (
        <div
          className="p-4 border-t border-white/10 opacity-0 animate-fadeIn"
        ...
        >
          <div className="flex items-center justify-between text-xs text-white/60">
            ...
            <Badge
              variant="secondary"
              className="text-xs bg-gradient-to-r from-emerald-500/20 to-blue-500/20 border-white/20 text-white/100 backdrop-blur-sm"
            >
              Beta
            </Badge>
          </div>
