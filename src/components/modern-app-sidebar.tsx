  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col md:hidden">
        <div className={`flex-1 pb-24 ${bgClass}`}>{children}</div>
        <FloatingBottomNav currentPath={currentPath} />
      </div>

      {/* Desktop Layout - Hidden on mobile */}
      <div className="min-h-screen hidden md:flex">
        <ModernSidebar currentPath={currentPath} />
        <div className={`flex-1 overflow-hidden ${bgClass}`}>{children}</div>
      </div>
    </>
  );
}
